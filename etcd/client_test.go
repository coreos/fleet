...
func TestNewClient(t *testing.T) {
	...
	for i, tt := range tests {
		_, err := NewClient(tt.endpoints, http.Transport{}, DefaultActionTimeout)
		if tt.pass != (err == nil) {
...
func TestClientCancel(t *testing.T) {
	act := Get{Key: "/foo"}
	c, err := NewClient(nil, http.Transport{}, DefaultActionTimeout)
	if err != nil {
...
func TestClientRedirectsFollowed(t *testing.T) {
	...
	c, err := NewClient([]string{"http://192.0.2.1:4001"}, http.Transport{}, DefaultActionTimeout)
	if err != nil {
...
func TestClientRedirectsAndAlternateEndpoints(t *testing.T) {
	...
	c, err := NewClient([]string{"http://192.0.2.1:4001", "http://192.0.2.2:4002"}, http.Transport{}, DefaultActionTimeout)
	if err != nil {
		t.Fatalf("NewClient failed: %v", err)
	}
...
func TestNilNilRequestHTTP(t *testing.T) {
	c := &client{[]url.URL{}, &nilNilTransport{}, DefaultActionTimeout}
	cancel := make(chan bool)
...
func TestRespAndErrRequestHTTP(t *testing.T) {
	c := &client{[]url.URL{}, &respAndErrTransport{}, DefaultActionTimeout}
	cancel := make(chan bool)
	resp, body, err := c.requestHTTP(nil, cancel)
	if err == nil {
		t.Error("unexpected nil error")
...
func TestTLSClientConfigNoCertificate(t *testing.T) {
...
func TestTLSClientConfigWithValidCertificateAndWithCA(t *testing.T) {
	ca := createTempfile(validCA)
	defer os.Remove(ca)
	key := createTempfile(validKey)
	defer os.Remove(key)
	cert := createTempfile(validCert)
	defer os.Remove(cert)

	config, err := TLSClientConfig(ca, cert, key)
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
	if config.InsecureSkipVerify {
...
func TestTLSClientConfigWithValidCertificateAndWithoutCA(t *testing.T) {
	key := createTempfile(validKey)
	defer os.Remove(key)
	cert := createTempfile(validCert)
	defer os.Remove(cert)

	config, err := TLSClientConfig("", cert, key)
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
	if config.InsecureSkipVerify {
...
func TestTLSClientConfigWithOnlyKeyfileIsAnError(t *testing.T) {
	key := createTempfile(corruptedKey)
	defer os.Remove(key)

	config, err := TLSClientConfig("", "", key)
	if err == nil {
...
func TestTLSClientConfigWithOnlyCertfileIsAnError(t *testing.T) {
	cert := createTempfile(validCert)
	defer os.Remove(cert)
...
func TestTLSClientConfigWithCorruptedCertificate(t *testing.T) {
	key := createTempfile(corruptedKey)
	defer os.Remove(key)
	cert := createTempfile(corruptedCert)
	defer os.Remove(cert)
...
func createTempfile(content string) string {
	f, err := ioutil.TempFile("", "certificate")
	if err != nil {
		panic(err)
	}
	ioutil.WriteFile(f.Name(), []byte(content), os.FileMode(0644))
	return f.Name()
}
...
-----END CERTIFICATE-----`
	...
	Corrupted
-----END RSA PRIVATE KEY-----`
	corruptedCert = `-----BEGIN CERTIFICATE-----
	Corrupted
-----END CERTIFICATE-----`
)
