...
func New(bootId string, publicIP string, publicInterface string, metadata map[string]string) *Machine {
	static := MachineState{bootId, publicIP, metadata}
	log.V(2).Infof("Created Machine with static state %s", static)
	m := &Machine{publicInterface: publicInterface, staticState: static}
	return m
}
...
func (m *Machine) RefreshState() *MachineState {
	state := CurrentState(m.publicInterface)
