...
flt provides a command-line tool called `fltctl`. The commands provided by
fltctl are analogous to those of systemd's CLI, `systemctl`.

## Get up and running

The `fltctl` binary is included in all CoreOS distributions, so it is as simple as SSH'ing in to your CoreOS machine and executing `fltctl`.

### Custom etcd Endpoint

fltctl currently requires direct communication with the etcd cluster that your flt machines are configured to use. Use the `--endpoint` flag to override the default of `http://127.0.0.1:4001`:

    fltctl --endpoint http://<IP:PORT> list-units

Alternatively, `--endpoint` can be provided through the `FLEETCTL_ENDPOINT` environment variable:

    FLEETCTL_ENDPOINT=http://<IP:[PORT]> fltctl list-units

In future, fltctl will communicate exclusively with a flt API endpoint, and will no longer require direct access to etcd.

### From an External Host

If you prefer to execute fltctl from an external host (i.e. your laptop), the `--tunnel` flag can be used to tunnel communication with your flt cluster over SSH:

    fltctl --tunnel <IP[:PORT]> list-units

One can also provide `--tunnel` through the environment variable `FLEETCTL_TUNNEL`:

    FLEETCTL_TUNNEL=<IP[:PORT]> fltctl list-units
...
If the external host requires a username other than `core`, the `--ssh-username` flag can be used to set an alternative username.
    ...
    fltctl --ssh-username=elroy list-units

Or

    FLEETCTL_SSH_USERNAME=elroy fltctl list-units
...
Be sure to install one of the [tagged releases](https://github.com/coreos/flt/releases) of `fltctl` that matches the version of flt running on the CoreOS machine.
...
See more about [configuring remote access](#remote-flt-access).

## Interacting with units

For information regarding the additional unit file parameters that modify flt's behavior, see [this documentation](https://github.com/coreos/flt/blob/master/Documentation/unit-files-and-scheduling.md).

### Explore existing units

List all units in the flt cluster with `fltctl list-unit-files`:
...
`fltctl list-unit-files` communicates what the desired state of a unit is, what its current state is, and where it is currently scheduled.

List the last-known state of flt's active units (i.e. those loaded onto a machine) with `fltctl list-units`:
...
```
$ fltctl start goodbye.service
Unit goodbye.service launched on 85c0c595.../172.17.8.102

$ fltctl stop goodbye.service
Unit goodbye.service loaded on 85c0c595.../172.17.8.102
```

If the unit does not exist when calling `start`, fltctl will first search for a local unit file, submit it and schedule it.

### Scheduling units

To schedule a unit into the cluster (i.e. load it on a machine) without starting it, call `fltctl load`:

```
$ fltctl load hello.service
...
Units can also be unscheduled, but remain in the cluster with `fltctl unload`.
The unit will still be visible in `fltctl list-unit-files`, but will have no state reported in `fltctl list-units`:

```
$ fltctl unload hello.service
...
Getting units into the cluster is as simple as a call to `fltctl submit`:

```
$ fltctl submit examples/hello.service
```
...
$ fltctl submit examples/*
```

Submission of units to a flt cluster does not cause them to be scheduled. 
The unit will be visible in a `fltctl list-unit-files` command, but have no reported state in `fltctl list-units`.

A unit can be removed from a cluster with the `destroy` command:

```
$ fltctl destroy hello.service
```
...
Once a unit is destroyed, state will continue to be reported for it in `fltctl list-units`.
Only once the unit has stopped will its state be removed.

### View unit contents

The contents of a loaded unit file can be printed to stdout using the `fltctl cat` command:

```
$ fltctl cat hello.service
[Unit]
...
Once a unit has been started, flt will publish its status. The systemd state fields 'LoadState', 'ActiveState', and 'SubState' can be retrieved with `fltctl list-units`. To get all of the unit's state information, the `fltctl status` command will actually call systemctl on the machine running a given unit over SSH:

```
$ fltctl status hello.service
...
The `fltctl journal` command can be used to interact directly with `journalctl` on the machine running a given unit:

```
$ fltctl journal hello.service
...
Describe all of the machines currently connected to the cluster with `fltctl list-machines`:

```
$ fltctl list-machines
MACHINE     IP           METADATA
...
The `fltctl ssh` command can be used to open a pseudo-terminal over SSH to a host in the flt cluster.
...
`fltctl ssh` will connect to the machine to-which the given unit is scheduled:

```
$ fltctl ssh hello.service
```

### Known-Hosts Verification

Fingerprints of machines accessed through fltctl are stored in `$HOME/.fltctl/known_hosts` and used for the verification of machine identity.
...
flt does not yet have any custom authentication, so security of a given flt cluster depends on a user's ability to access any host in that cluster. The suggested method of authentication is public SSH keys and ssh-agents. The `fltctl` command-line tool can assist in this by natively interacting with an ssh-agent to authenticate itself.
...
[d]: https://github.com/coreos/flt/blob/master/Documentation/deployment-and-configuration.md
...
To verify the ssh-agent and remote hosts are properly configured, simply connect directly to a host in the flt cluster using `ssh`. Configure `fltctl` to tunnel through that host by setting the `--tunnel` flag or exporting the `FLEETCTL_TUNNEL` environment variable:

```
$ fltctl --tunnel 192.0.2.14:2222 list-units
...
$ FLEETCTL_TUNNEL=192.0.2.14:2222 fltctl list-units
...
```

## Vagrant

Things get a bit more complicated when using [vagrant][v], as access to your hosts is abstracted away from the user. This makes it a bit more complicated to run `fltctl` from your local laptop, but it's still relatively easy to configure.
...
Now, let's get `fltctl` working with these parameters:
